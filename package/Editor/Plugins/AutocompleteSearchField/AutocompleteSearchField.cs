...
namespace Disassembler.Editor.Plugins.AutocompleteSearchField
{
	[Serializable]
	public class AutocompleteSearchField
	{
		...
		void Draw(bool asToolbar)
		{
            ...
            if(results.Count > 0 && showResults)
                GUILayout.Space(Styles.resultHeight * Mathf.Min(maxResults, results.Count));
		}
        ...
        {
            //Debug.Log("do results " + rect + " " + results.Count + " ," + showResults);
            if (results.Count <= 0 || !showResults) {
                return;
            }
            ...
            scrollOffset = Mathf.Min(scrollOffset, results.Count - maxResults);
            scrollOffset = Mathf.Max(scrollOffset, 0);

            //Debug.Log($"{scrollOffset}, results: {results.Count}, max: {maxResults}");

			for (var i = 0 + scrollOffset; i < results.Count && i < scrollOffset + maxResults; i++)
			{
            ...
            {
                scrollOffset += (int) current.delta.y;
                //Debug.Log(scrollOffset);
            }
		}
        ...
        {
            if (onSelectedIndexChanged != null) onSelectedIndexChanged(results[index]);
        }

		bool HasSearchbarFocused()
		{
            var focussed = GUIUtility.keyboardControl == searchField.searchFieldControlID;
            if (focussed)
            {
                if (!showResults)
                {
